...
export function TokenWaterfall({ events }: Props) {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 overflow-x-auto relative">
      <h3 className="text-sm font-medium text-gray-300 mb-4">Token Usage Waterfall</h3>
      ...
      {hoveredIdx !== null && llmEvents[hoveredIdx] && (
        <div className="absolute top-12 right-4 bg-gray-800 border border-gray-700 rounded-lg p-3 shadow-xl z-50 min-w-[200px]">
          <div className="flex items-center gap-2 mb-2">
            <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: agentColorMap[llmEvents[hoveredIdx].agentName] }} />
            <span className="text-sm font-medium text-white">{llmEvents[hoveredIdx].agentName}</span>
          </div>
          <div className="space-y-1 text-xs text-gray-400">
            <p>Tokens: <span className="text-white">{llmEvents[hoveredIdx].tokens.toLocaleString()}</span></p>
            <p>Cost: <span className="text-white">${llmEvents[hoveredIdx].cost.toFixed(5)}</span></p>
            <p>Type: <span className="text-white">{llmEvents[hoveredIdx].type}</span></p>
            <p>Time: <span className="text-white">{new Date(llmEvents[hoveredIdx].timestamp).toLocaleTimeString()}</span></p>
          </div>
        </div>
      )}

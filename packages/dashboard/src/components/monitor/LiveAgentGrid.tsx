...
const STATUS_CONFIG: Record<LiveAgentStatus['status'], { border: string; icon: React.ReactNode; label: string }> = {
  ...
  idle: { border: 'border-gray-700', icon: <Coffee className="w-4 h-4 text-gray-500" />, label: 'Idle' },
...
export function LiveAgentGrid({ agents }: Props) {
  return (
    <div>
      <h2 className="text-sm font-medium text-gray-400 mb-3">Active Agents</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {agents.map(agent => {
          const config = STATUS_CONFIG[agent.status];
          return (
            <div key={agent.agentId} className={`bg-gray-900 border-2 ${config.border} rounded-lg p-4 transition-colors`}>
              ...
              {agent.currentStep && (
                <p className="text-xs text-gray-400 mb-3 truncate">{agent.currentStep}</p>
              )}
